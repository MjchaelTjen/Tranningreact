...
export default class BaiTapRenderDanhSachPhim extends Component {
    ...
    renderDSPhim = () => {
        let arrRenderDSPhim = mangPhim.map((phim, index) => {
            return (
                <div className="col-lg-4 col-md-6 mb-4" key={index}>
                    <div className="card h-100">
                        <img className={`card-img-top ${style.hinhAnhPhim}`} src={phim.hinhAnh} alt />
                        <div className="card-body">
                            <h4 className="card-title bg-dark" style={{ height: "60px" }}>{phim.tenPhim.length > 50 ? <span>{this.tenPhim.substr(0, 50)}...</span> : <span>{phim.tenPhim}</span>}</h4>
                            <p className={`card-text ${style.moTaPhim}`}>{phim.moTa.length > 150 ? <span>{phim.moTa.substr(0, 150)}...</span> : <span>{phim.moTa}</span>}</p>
